...
class VmMediaHandler {
	...
	static private function isImage($file_extension=0){
		...
		function processAction($data){
			...
			$pathImg = JPATH_ROOT.DS."images/stories/virtuemart/product/";
			$pathResized = JPATH_ROOT.DS."images/stories/virtuemart/product/resized/";
			...
			$db = JFactory::getDBO();
				$files = $_FILES['multiUpload'];
				foreach ($files as $k => $v) {
					foreach ($v as $k2 => $v2) {
						if(JFile::upload($files['tmp_name'][$k2],$pathImg.$files['name'][$k2])){
						$createdImage = new Img2Thumb($pathImg.$files['name'][$k2], 90, 90, $pathResized.$files['name'][$k2], $maxsize, $bgred, $bggreen, $bgblue);

 					      $SQL = "INSERT INTO `#__virtuemart_medias`
 					      (`virtuemart_vendor_id`, `file_title`,
 					       `file_description`, `file_meta`, `file_mimetype`,
 					        `file_type`, `file_url`, `file_url_thumb`, 
 					        `file_is_product_image`, `file_is_downloadable`, 
 					        `file_is_forSale`, `shared`, `published`, 
 					        `created_by`, `modified_by`,`locked_by`) 
 					      VALUES (1,".$db->quote($files['name'][$k2]).",
 					      	'','',".$db->quote($files['type'][$k2]).",
 					      	'product',".$db->quote("images/stories/virtuemart/product/".$files['name'][$k2]).",
 					      	".$db->quote("images/stories/virtuemart/product/resized/".$files['name'][$k2]).",
 					      	0,0,0,0,1,0,0,0)";
			                ...
			                if($inserted_id){
			                	$SQL = "INSERT INTO `#__virtuemart_product_medias`(
			                		`virtuemart_product_id`,
			                		`virtuemart_media_id`) 
			                	VALUES (".JRequest::getVar('virtuemart_product_id').",".$inserted_id.")";
			                	$db->setQuery($SQL);
			                	$db->query();
		                	}    
						}
				...
				$app = JFactory::getApplication();
				//die("http://".$_SERVER['HTTP_HOST']."/administrator/index.php?option=com_virtuemart&view=product&task=".JRequest::getVar('task')."&virtuemart_product_id[]=".JRequest::getVar('virtuemart_product_id'));
				header("Location: http://".$_SERVER['HTTP_HOST']."/administrator/index.php?option=com_virtuemart&view=product&task=edit&virtuemart_product_id[]=".JRequest::getVar('virtuemart_product_id'));
				exit;
			}
...
// 			$this->file_is_forSale = 0;
	...
	$html .='</td>';
			...
			$html .= '<br />Качване на файлове <input name="multiUpload[]" type="file" multiple="multiple" />';
